...
standalone or in conjunction with L<HTTP::State> for a fast and up to date
...
last access time fields, if they are present.

This performs the reverse of the C<$store_and_offset> argument in
C<encode_set_cookie> to covert from local time to GMT times. This is used by
L<HTTP::State> to load cookies in local time for storage compatible with
L<HTTP::CookieJar>.
...
If the the C<$store_and_offset> argument is defined, internal fields for the
