...
HTTP::State - RFC6265bis-draft and CHIPS Cookie Jar for HTTP Clients
...
An up to date cookie processing module, implementing a RFC6265bis-draft "cookie
jar" with additional CHIPS (Partitioning) support.  At the time of writing the
...
Partitioning of cookies is also supported but must be enabled when a cookie jar
is created.  User agent must provided the scheme and host of the first party
site as the partition key.
