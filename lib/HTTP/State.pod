...
of cookies. This modules is a cookie jar providing the same up to date thinking
to Perl user-agents. 
...
default value required to work with these new features where needed.

Long term, existing user agents will need to be modified in order to take full
