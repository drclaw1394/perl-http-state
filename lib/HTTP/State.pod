...
Web browsers are constantly improving privacy and limiting the traceability of
cookies. This modules is a cookie jar providing the same up to date thinking to
Perl user-agents. 

The algorithm implemented is derived from RFC6265bis-draft and Cookies Having
Independent Partitioned State (CHIPS). As these are not finalised standards, it
is to be expected this module may change behaviour to keep up to date.

To aid adoption, a L<HTTP::CookieJar> compatible API is implemented using
default values (specific to each jar instance) where needed.

Long term, existing user agents would need to be modified to take full
advantage of this module. Not only are there differences in the API compared to
popular cookie jars, but conceptually the notion of 'browsing context' needs to
be implemented.
...
At the time of writing RFC6265vis at draft '12'. As the RFC progresses this
...
Cookies are restricted expire 400 days in the future, instead 
...
They are used to describe the intent and context of a request initiated by a
user-agent in a space efficient manner. Multiple flags are ORed together into a
bit field. This bit field is required for storing and retrieval of cookies
throughout this API.
...
host, using the same scheme for example.
...
site, which is used as a partition key. Cookies are stored to this partition if
the jar was instantiated with the C<enable_partition> option.
...
=head3 retrieve_cookies
  ...
  $jar->retrieve_cookies($request_url, $parition, $flags); 
