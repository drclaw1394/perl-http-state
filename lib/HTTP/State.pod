...
Secure lookup and encode (RFC6265bis) from user agent parameters:
  ...
  my $request_url="http://test.example.com";
  my $same_site_status="same-site"; #or "cross-site";
  my $type="HTTP";                  # or "non-HTTP";
  my $safe=1;                       # or 0 or undef
  my $string=$jar->encode_request_cookies($request_url);
...
An up to date and fast (20x-30x than HTTP::CookieJar) pure Perl cookie
processing module, implementing a RFC6265-bis-draft based "cookie jar" for HTTP
client side applications.

It provides a compatible interface to existing cookie jars, for a 'drop in'
upgrade of performance and ease of migration into L<HTTP::Tiny> and L<LWP>.

With limited changes to existing code, it provides some of the features of
RFC6265bis:
...
=item Upper limit of cookie expiry dates

=back

However, with a few additional arguments, a user-agent can specify the
'browsing context' when retrieving cookies for full RFC6265bis processing:
...
This does require the user agent to do additional book keeping of what is same
site in respect to top level navigation, reloading, document etc. This module
simply needs to know if request is same site (or not), http or api request, and
if the method is safe (ie not POST or PUT).

This module does not export any symbols and is intended to be use via an object
orientated interface. For explicit encoding and decoding of cookie strings,
please see the companion L<HTTP::State::Cookie> module.
...
When this flag is set, request is considered "HTTP". When unset,
request is considered "non-HTTP".

=head3 FLAG_SAFE_METH

When this flag is set, request is considered "safe". When unset,
...
RFC6265bis 'cookie retrievel', and encodes them into a string suitable for use
